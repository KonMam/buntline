...
// are browser-side, and settings persist per browser in localStorage —
// but registering it makes the feature visible and toggleable in the
// modules store. Disabling it stops the browser from subscribing to the
// global event stream, so a disabled module truly costs nothing.
